 ...
 */
abstract class SyncResourceBaseBackup extends PluginBase implements SyncResourceInterface, ContainerFactoryPluginInterface {
  ...
  protected function fetchData($additional = []) {
    try {
      ...
      if (!empty($data) || $this->shouldRunOnEmpty()) {
        ...
        if ($fetcher instanceof SyncFetcherPagedInterface && count($data) === $fetcher->getSize()) {
  ...
  /**
   * Prepare the data and allow for altering and filtering.
   *
   * @param array $data
   *   An array of data.
   *
   ...
   *
   * Be careful for if the fetcher returns a large number of results this can
   * time out.
   */
  ...
  public function fetchPage(array $batch_data) {
    try {
      ...
      // continue filtering.
      $expect_more_data = count($new_data) === $fetcher->getSize();
  ...
  public function loadEntity(array $data) {
    ...
    if ($entity_type && $bundle) {
      $entity = $this->syncEntityProvider->getOrNew($this->id($data), $entity_type, $bundle, $this->getInitialValues($data), $this->getGroup($data));
    }
    return $entity;
  }
    ...
    try {
      ...
      if (!empty($results)) {
        foreach ($results as $sync) {
          $context['%entity_id'] = $sync->id;
          $this->drushLog('Queue Cleaning: %id [%entity_type:%bundle:%entity_id]', $context);
          $this->queue->createItem([
            'plugin_id' => $this->getPluginId(),
            'op' => 'clean',
            'data' => [
      ...
      $context['%error'] = $e->getMessage();
      $this->drushLog('Cleaning Fail: %id [%entity_type:%bundle] %error', $context, 'error');
  ...
  public function clean(array $data) {
    ...
    try {
      ...
      if ($entity) {
        if ($this->syncAccess($entity)) {
          $context['%entity_id'] = $entity->id();
          $this->drushLog('Cleaning: %id [%entity_type:%bundle:%entity_id]', $context);
          $this->cleanExecute($entity, $sync, $context);
          $remaining = $this->queue->numberOfItems();
          if ($remaining % 50 === 1) {
            $this->drushLog('Cleaning... [Remaining: @remaining | Success: @success | Skipped: @skip | Failed: @fail]', [
              '@remaining' => $remaining,
              '@success' => $this->getProcessCount('success'),
              '@skip' => $this->getProcessCount('skip'),
              '@fail' => $this->getProcessCount('fail'),
      ...
      $this->drushLog('Cleaning Skip: %id for %entity_type:%bundle', $context, 'warning');
